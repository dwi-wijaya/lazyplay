    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 mt-8">
        <div class='group relative flex flex-col min-h-56 w-full' v-for="(video, index) in videos" :key="index">
            <div class='duration-500 relative h-full rounded-xl overflow-hidden'>
                ...
                <div class="absolute z-10 top-0 left-0 flex  justify-center items-center w-full h-full">
                    <button @click="AddToQueue(video)" :disabled="isCooldown" class="btn z-10 opacity-0 group-hover:opacity-100">
                        <i class="fad fa-signal-stream"></i>
                        {{ isCooldown ? `Cooldown (${cooldownTime}s)` : 'Add To Queue' }}
                    </button>
                    <div class="absolute top-0 left-0 bg-black opacity-0 group-hover:opacity-50 base-transition w-full h-full"></div>
                </div>
                <div class='absolute inset-0 bg-gradient-to-b from-black/30 to-black opacity-90 transition-opacity duration-300'></div>
            </div>
            <div class='absolute flex flex-col justify-end p-5 space-y-4 h-full w-full group-hover:hidden'>
                <div class='flex flex-col justify-end'>
                    <div class='flex flex-col space-y-3 text-neutral-300'>
                        <h3 class='line-clamp-1 leading-5 !mt-1 font-medium text-neutral-200 group-hover:underline group-hover:underline-offset-4 '>
                            {{ video.snippet.title }}
                        </h3>
                        <div class='flex gap-1 items-center text-neutral-400 !mt-2'>
                            <i class='fal fa-calendar text-xs' />
                            <span class='text-xs ml-0.5'>{{ useDateFormat(video.snippet.publishedAt, 'DD MMM, YYYY ') }}</span>
                        </div>
...
import { parseArtist, getChannelDetails } from '@services/youtube.js';
